...
import { login, getPlan, completePlan, getRecs, makeNudge, ingest, listMetrics, setSteps } from './api'
...
export default function App(){
  ...
  // --- derive dashboard stats from metrics/plan ---
  const nowISODate = new Date().toISOString().slice(0,10)
  const stepsToday = useMemo(()=>metrics
    .filter(m=>m.metricType==='Steps' && m.ts.slice(0,10)===nowISODate)
    .reduce((s,m)=>s + Number(m.value||0), 0), [metrics])
  ...
  const activeMinutes = useMemo(()=> (plan?.items||[])
      .filter(i=>i.type==='Workout')
      .reduce((s,i)=>s + (Number(i.durationMin)||0), 0), [plan])

  // simple demo wellness score (0–100)
  const wellness = useMemo(()=>{
    const s = clamp(stepsToday/100, 0, 100)       // 10,000 steps ~ 100
  ...
  const addSample = async ()=>{
    ...
    }catch(e){ console.error(e); setErr('Failed to add metrics') }
  }

  // NEW: save steps (one-per-day upsert)
  const saveSteps = async ()=>{
    try{
      ...
      setMetrics(await listMetrics())
      setStepsInput('')
    }catch(e){ console.error(e); setErr('Failed to save steps') }
  }
    ...
    <div className="container">
      ...
      <section className="wellness-row">
        <div className="wellness-card">
          ...
          <div className="ring-caption">{wellness >= 80 ? 'Excellent Progress!' : wellness >= 60 ? 'Great Momentum!' : 'Let’s build consistency'}</div>
        ...
        <div className="quick-actions card">
          ...
          <div className="qa-grid">
            <button className="btn qa start"
              onClick={async ()=>{ await completePlan(); setPlan(await getPlan()) }}>
              Start Workout
              <small>Begin your AI-recommended session</small>
            </button>
            <button className="btn qa ask"
              onClick={async ()=>{ await makeNudge(); setRecs(await getRecs()) }}>
              Ask AI Coach
              <small>Get a motivational nudge</small>
            </button>
            <button className="btn qa plan"
              onClick={refresh}>
              Plan Week
              <small>Regenerate today’s plan</small>
            </button>
            <button className="btn qa goals"
              onClick={addSample}>
              Set Goals
              <small>Simulate more data</small>
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="grid">
        <PlanCard plan={plan} onComplete={async ()=>{ await completePlan(); setPlan(await getPlan()) }} />

        <div className="card">
          <h3>Recommendations</h3>
          <button className="btn" onClick={async ()=>{ await makeNudge(); setRecs(await getRecs()) }}>Generate Nudge</button>
          <ul>{recs.map((r, i)=><li key={i}>{r.message} <em>({new Date(r.ts).toLocaleTimeString()})</em></li>)}</ul>
        </div>
        ...
        <div className="card">
          ...
          <ul style={{marginTop:'.5rem'}}>{metrics.slice(0,12).map((m,i)=>
            <li key={i}>{m.metricType}: {m.value} <em>({new Date(m.ts).toLocaleTimeString()})</em></li>
          )}</ul>
        </div>
      </section>
